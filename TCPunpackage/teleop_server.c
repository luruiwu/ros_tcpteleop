...
#define MAXBYTEMUN  50  //通信内容的最大长度
#define MyPort 8888
int main()
{
	...
	tv.tv_usec = 0;
	/*开启发送超时*/
	//setsockopt(sock_fd，SOL_SOCKET，SO_SNDTIMEO，(char *)&interval,sizeof(struct timeval));
	/*开启接收超时*/
	setsockopt(new_fd,SOL_SOCKET,SO_RCVTIMEO,&tv,sizeof(struct timeval));
	/*开启reuseaddr*/
	int on=1;
	...
	printf("server:got connection from IP= %s prot= %d \n",inet_ntoa(client_addr.sin_addr),ntohs(client_addr.sin_port));//连接成功，打印客户机IP地址和端口号
        ...
        */
	if(send(new_fd,"Hello client,I am 192.168.31.96!\n",50,0)==-1)  //192.168.31.96为子进程IP，可更改
		perror("send failed");
	...
	else   //f=父进程用于接收数据
	{
		...
		char last_recvbuf[MAXBYTEMUN];
		//close(new_fd);//父进程不需要连接，所以关闭连接套接字
		while(1)
		{
			memset(recvbuf,0,sizeof(recvbuf));
			n=recv(new_fd,recvbuf,MAXBYTEMUN,0);
			if(n==0)
			{
				printf("peer closed\n");    
				break;
			...
			else if(n<0)
			{
				if( n==-1 && errno==EAGAIN)
				{
					fputs(last_recvbuf,stdout);
					continue;
				}
				else
					perror("read from client error(timeout)");    
			}
			memcpy(&last_recvbuf,&recvbuf,sizeof(recvbuf));
